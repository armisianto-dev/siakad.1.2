...
class Auth extends CI_Controller {
	...
	function edit_user($id){
		...
		if (isset($_POST) && !empty($_POST)){
			...
			if ($this->form_validation->run() === TRUE){
				$data = array(
					...
					'tbl_asal'      => $this->input->post('tbl_asal'),
				);
		...
		$this->data['tbl_asal'] = array(
			'name' => 'tbl_asal',
			'id'   => 'tbl_asal',
			'type' => 'hidden',
			'value' => $this->form_validation->set_value('tbl_asal', $user->tbl_asal),
			'class' => 'form-control input-sm',
			'placeholder' => 'Namapengguna',
		);
		$this->data['email'] = array(
	...
	function edit_group($id){
		...
		$this->data['group_name'] = array(
			...
			'id'    => 'name',
			'type'  => 'text',
			'value' => $this->form_validation->set_value('group_name', ucwords(str_replace('_', ' ', $group->name))),
			'class' => 'form-control input-sm',
			'placeholder' => 'Nama Grup',
			'readonly' => 'readonly',
		);
		$this->data['group_name_hide'] = array(
			'name'  => 'group_name',
			'id'    => 'group_name',
			'type'  => 'hidden',
	...
	function create_roles_cat(){
		...
		//jika tidak termasuk grup admin atau super_admin
			...
			if ($this->form_validation->run() == TRUE){			
				...
				$this->layout->back('auth/create_roles_cat', $this->data);
			}
		}
	}
	//edit role category
	function edit_roles_cat($id = ''){
		...
		//jika tidak termasuk grup admin atau super_admin
			...
			$this->layout->back('auth/edit_roles_cat',$data);
		}
	}
	//edit kategori role
