...
class Ion_auth_model extends CI_Model
{
	...
	public function user_a($id = NULL)
	{
		$this->trigger_events('user');
		...
		$this->select("guru.nama,rc_users.username,guru.foto");
		$this->limit(1);
		$this->order_by('id', 'desc');
		$this->db->join('guru','guru.nip=rc_users.username');
		$this->where($this->tables['users'].'.id', $id);
	...
	public function user_b($id = NULL)
	{
		$this->trigger_events('user');
		...
		$this->select("siswa.nisn,siswa.nama,rc_users.username,siswa.foto");
		$this->limit(1);
		$this->order_by('id', 'desc');
		$this->db->join('siswa','siswa.nis=rc_users.username');
		$this->where($this->tables['users'].'.id', $id);
