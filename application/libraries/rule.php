    ...
    function sidebar_menu(){
        ...
        if($arr1->num_rows()>0){
            foreach($arr1->result_array() as $key => $value){
                ...
                /*$q3 = 'SELECT * FROM (
                    SELECT category, url FROM '.$this->CI->tables['permissions'].' 
                    join '.$this->CI->tables['roles'].' ON '.$this->CI->tables['roles'].'.id = '.$this->CI->tables['permissions'].'.role_id
                    join '.$this->CI->tables['roles_category'].' ON '.$this->CI->tables['roles_category'].'.id = '.$this->CI->tables['roles'].'.category_id
                    WHERE group_id IN (SELECT group_id FROM '.$this->CI->tables['users_groups'].' WHERE user_id = '.$user->id.')
                    AND rule != "0000") as q
                ...
                foreach ($arr3->result_array() as $isi) {
                    if ($fullUrl == site_url($isi['url']) && $value['category'] == $isi['category']){echo 'active'; $active = '1';}else{$active = '0';}
                    $akuuuh = site_url($isi['url']);
                }*/
                foreach ($arr2->result_array() as $v) {
                    if ($url_controller == site_url($v['url'])) {echo 'active';}
                }
                echo '">
                <a href="#">';
                /*foreach ($arr2->result_array() as $url) {
                    $full_url = site_url($url['url']);
                }*/
                echo '<i class="';
                    $icon = array();
                ...
                echo '"';
                    //if($active == '1'){echo ' style="color:green;font-weight:bold;"';}
                echo '></i>
                    <span';
                        //if($active == '1'){echo ' style="color:green;font-weight:bold;"';}
                    echo '>'; 
                ...
                //if($active == '1'){echo ' style="color:green;"';}
                echo '></i>
                </a>';
                echo '<ul class="treeview-menu">';
                    foreach ($arr2->result_array() as $val_menu) {
                        echo'<li ';
                        if ($url_controller == site_url($val_menu['url'])) {echo 'class="current"';}
